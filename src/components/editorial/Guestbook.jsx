    ...
    <Section title="GUESTBOOK">
      <div className="mx-auto max-w-[390px]">
        <form onSubmit={handleSubmit} className="space-y-5 text-left">
          ...
          {error && <p className="text-center text-[12px] text-ink/65">{error}</p>}
        ...
        <div className="mt-12 space-y-4 text-left">
          {loading ? (
            <p className="text-center text-[12px] text-ink/40">LOADING</p>
          ) : entries.length === 0 ? (
            <p className="text-center text-[12px] text-ink/40">첫 메시지를 남겨주세요.</p>
          ) : (
            <>
              {entries.map((entry) => (
                <article key={entry.id} className="border-t border-ink/12 py-5 last:border-b">
                  <div className="flex items-baseline justify-between gap-4">
                    <p className="text-[14px] font-medium text-ink">{entry.name}</p>
                    <p className="font-mono text-[10px] text-ink/35">{formatDate(entry.created_at)}</p>
                  </div>
                  <p className="mt-3 whitespace-pre-wrap text-[14px] leading-relaxed text-ink/72">{entry.message}</p>
                </article>
              ))}
              {hasMore ? (
                ...
                <p className="pt-2 text-center font-mono text-[10px] tracking-[0.2em] text-ink/30">END</p>
              )}
            </>
          )}
        </div>
