...
export default function HandwritingMarried({
  ...
  useLayoutEffect(() => {
    ...
    const next = lengths.map((len) => {
      const duration = Math.max(0.12, len / pxPerSec);
      const entry = { delay: cursor, duration };
      cursor += duration * (1 - overlap);
      return entry;
    });
    setTimings(next);
  }, [pxPerSec, overlap, startDelay, replayKey]);
              ...
              <g key={i} transform={p.transform}>
                <motion.path
                  ...
                  transition={
                    t
                      ? {
                          pathLength: { duration: t.duration, delay: t.delay, ease: 'easeInOut' },
